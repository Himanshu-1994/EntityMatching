...
def to_str(ent1, ent2, label, summarizer=None, max_len=256, dk_injector=None):
    ...
    return new_ent1 + '\t' + new_ent2 + '\t' + label
...
def classify(sentence_pairs, model,
    ...
    return pred, all_logits, all_targets
...
def predict(input_path, output_path, config,
    ...
    def process_batch(rows, pairs, writer):
        predictions, logits, targets = classify(pairs, model, lm=lm,
                                       max_len=max_len,
                                       threshold=threshold)
        # try:
        ...
        for row, pred, score, target in zip(rows, predictions, scores, targets):
    ...
    with jsonlines.open(input_path) as reader,\
        ...
        for idx, row in tqdm(enumerate(reader)):
            pairs.append(to_str(row[0], row[1], row[2], summarizer, max_len, dk_injector))
            rows.append(row)
